...
- **inventory:** Inventory defines which servers Ansible should manage and groups them based on environments.
- **roles/ :** Roles make Ansible automation modular, reusable, and easier to maintain.

 > tasks/main.yml → Nginx installation, service start, configuration etc.

 > templates/nginx.conf.j2 → Dynamic Nginx configuration using Jinja2 variables.

- **playbooks :** Playbooks define what configuration or deployment should happen on which hosts.
- **group_vars :** group_vars allows us to separate configuration data from automation logic.
...
- **I follow a modular Ansible structure. Inventory contains environment-wise servers such as dev and prod. Roles contain reusable components like Nginx and Docker. Playbooks define the overall configuration and deployment workflow. Group_vars contains environment or group-specific variables, while ansible.cfg contains project-level Ansible configuration. This structure makes the automation reusable, maintainable, and easy to manage across environments.
